          ...
          {slips.map((slip) => (
            <tr key={slip.slip_id} className="group hover:bg-slate-50/50 transition-colors">
              <td className="px-6 py-4">
                <div className="font-bold text-slate-900">{slip.month_year}</div>
              ...
              <td className="px-6 py-4">
                <div className="font-black text-slate-900">₹{parseFloat(slip.final_amount).toLocaleString()}</div>
              </td>
              <td className="px-6 py-4">
                <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                  ...
                  slip.status === 'draft' ? 'bg-orange-100 text-orange-700' :
                  'bg-slate-100 text-slate-500'
                }`}>
                  {slip.status === 'released' ? <CheckCircle2 size={10} /> : 
                   slip.status === 'approved' ? <Clock size={10} /> : 
                   slip.status === 'draft' ? <Clock size={10} /> : <Ban size={10} />}
                  {slip.status}
                </div>
              </td>
              <td className="px-6 py-4">
