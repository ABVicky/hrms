...
export default function LeavesPage() {
    ...
    const loadRequests = async () => {
        try {
            const res = await appsScriptFetch('/get-leave-requests', { employee_id: user?.employee_id });
            setRequests(res || []);
        } catch (err) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-1 md:px-0">
                <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs md:text-sm text-slate-500 font-medium tracking-tight">Apply for or track your leave requests.</p>
                    </div>
                </div>
            </div>
